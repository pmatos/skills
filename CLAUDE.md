...
Slash command `/pm-triage` that runs a per-project decision session over open GitHub issues labeled `needs-triage`, `human-needed`, or `sym:human-needed`. Parallel read-only sub-agents analyze each issue first — resolved by a merged PR, superseded, or still genuinely needing a call — and only genuine calls reach the user, one at a time via `AskUserQuestion`, with an explicit stop-for-today option on every question. Each decision is written straight back to GitHub as an issue comment plus a label removal, so the label itself is the queue and a later run naturally resumes wherever the last one stopped — no local state file.
