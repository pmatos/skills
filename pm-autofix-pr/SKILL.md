...
| Reply on an inline review thread | `gh api repos/{owner}/{repo}/pulls/{n}/comments/{id}/replies --input -` (JSON built via `jq` from a temp file, to avoid shell-escaping the reviewer-authored body) | MCP's typed body field, if connected — sidesteps escaping entirely |
...
3. Auto-detect the PR. Try the following resolution strategies in order and stop at the first that yields exactly one open PR whose `head.ref` matches the local branch:
   - **3a. Origin lookup.** Parse `git remote get-url origin` to `{owner}/{repo}` (strip `git@github.com:`, `https://github.com/`, trailing `.git`). Run `gh pr list -R {owner}/{repo} --head <branch> --state open --json number,url,headRefName,baseRefName`.
   - **3b. Upstream lookup (fork workflow).** If step 3a returned no PRs and `git remote get-url upstream` exists, parse it the same way to `{upstream_owner}/{upstream_repo}` and run `gh pr list -R {upstream_owner}/{upstream_repo} --head <branch> --state open --json number,url,headRefName,baseRefName` (`gh pr list`'s `--head` filter matches on branch name only — it does not support the `owner:branch` qualifier — but scoping the query to the fork's declared upstream via `-R` already disambiguates which repo's PR list to search).
...
- Inline review thread: reply to `commentId = latestReviewerComment.databaseId` — primary path `gh api repos/{owner}/{repo}/pulls/<n>/comments/<commentId>/replies --input -` with the body written to a temp file and wrapped into JSON via `jq`; if a GitHub MCP is already connected, its reply tool may be used instead (its typed body field sidesteps the escaping step entirely).
