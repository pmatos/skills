...
- Check for ahead commits directly — run `git log origin/<head.ref>..HEAD --oneline` (the same check Step 5g's condition 4 uses) — rather than relying on `COMMITTED_ITEMS` alone: it is scoped to this iteration and reads empty whenever Step 5b didn't run this iteration, even when a *previous*, interrupted run already made commits that were never pushed (the case Step 3's "nothing to fix" short-circuit routes here to handle). If `COMMITTED_ITEMS` is empty for this iteration **and** that command's output is empty, there is nothing to push — **skip directly to Step 5g** (re-fetch and check fixed point), **not** 5f. An iteration that only emits REJECT replies, DEFER replies, or `automated-fix-failed` DEFER conversions creates no new commits and therefore no new CI events, so waiting for CI in 5f would just spin until `CI_TIMEOUT` and exit as a failure even when we have actually reached the fixed point.
