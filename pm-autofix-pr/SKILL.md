...
Runs only when Step 5g's conditions 1-4 already print `PASS` and `ALL_COMMITTED_ITEMS` is non-empty (see condition 5's own text for the vacuous-empty case, and for why no point reviewing a diff that other in-flight work is about to change).
