...
**Bound.** Beyond the exact-repeat stale-loop trigger in step 4, a reviewer that keeps finding *different* plausible-sounding issues every pass (never repeating verbatim) isn't caught by that dedup — but every spawn here, and any CI re-run a fix triggers via 5d/5f, still deducts from `TOTAL_CI_BUDGET_REMAINING` like everything else in this loop, so that case still bottoms out in the existing `ci-timeout` exit rather than running forever.
