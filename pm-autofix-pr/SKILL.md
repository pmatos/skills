...
Not every review comment deserves a code change in this PR, and not every rejected comment is worthless. Before touching code, evaluate every review thread, review summary, and PR conversation comment on its merits: a **primary evaluator** forms the verdict, cross-checked by **the advisor** when one is available in this session (see Step 0a). Each item is triaged into exactly one of:
...
- **A native subagent tool** (the `Agent`/`Task` tool) is preferred for the primary evaluator in Step 4 and Step 5i, giving each verdict a clean context rather than the loop's own accumulated history. Its absence never blocks the skill — both steps fall back to evaluating inline instead (slower, not different).
- **The advisor** — whatever this session's harness calls its second-opinion/reviewer-model feature — is optional and cross-checks the primary evaluator's verdict in Step 4 and Step 5i when present. Its absence never blocks the skill; those steps proceed on the primary evaluator's verdict alone (see each step's degraded-mode handling).
...
`<tmpfile>` and `<titlefile>` below always mean a file allocated via `mktemp` for that specific write (e.g. `mktemp /tmp/reply-body-XXXXXX`) — never a fixed literal path. Two concurrent `/pm-autofix-pr` invocations on the same host would otherwise race on a shared filename, letting one invocation's generated body or title get overwritten by the other's before `gh` reads it. Capture the exact path `mktemp` returns, reuse it across the write and the `gh` call, then `rm -f` it afterward.
...
### Step 0a: Detect available evaluation capabilities

This skill runs under any harness. It never checks which one — only what it can call from where it happens to be running:

- **Native subagent tool** — if a native `Agent`/`Task` tool is available, set `SUBAGENT_TOOL = true`; Step 4 and Step 5i's primary evaluator spawns a clean-context subagent with it (`Agent` tool, `subagent_type: "Explore"` or an equivalent read-capable general agent). If not, set `SUBAGENT_TOOL = false`; those steps evaluate inline instead — slower, not different.
- **Advisor** — if this session has a consultable advisor (however this harness exposes that — an `advisor` tool, a documented advisor feature, etc.), set `ADVISOR = true`. If not, set `ADVISOR = false`. Neither state blocks the skill; Step 4 and Step 5i both define their `ADVISOR = false` behavior explicitly.

Neither check requires installing anything or shelling out to another CLI. Record both flags now; every later step reads them rather than re-probing.
...
Also initialize `REGRESSION_FIXES = []` and `LAST_REGRESSION_FINDINGS = []` here, same run-wide-persistence reasoning: `REGRESSION_FIXES` accumulates one entry per self-caught regression fix for Step 7's summary, the same way `ALL_COMMITTED_ITEMS` does for reviewer-triggered fixes, and `LAST_REGRESSION_FINDINGS` must survive across Step 5i invocations so a finding that recurs after being "fixed" can be recognized (Step 5i's stale-loop bound) rather than re-derived fresh each time.

Also initialize `BLOCKED_REGRESSIONS = []` here, for the same run-wide-persistence reasoning: it accumulates one entry per synthetic `regression:<n>` FIX that Step 5b could not land (pre-commit still failing after its one sub-fix), for Step 7's summary — see Step 5i step 4 for why these cannot go through Step 5b's normal `automated-fix-failed` DEFER conversion the way a reviewer-triggered blocked FIX does.

Also initialize `MERGE_COMMITS = []` here, for the same run-wide-persistence reasoning as `ALL_COMMITTED_ITEMS`: Step 5h appends to it across however many merge-conflict resolutions happen over the run, and both Step 5g's condition 5 and Step 5i's regression-review scope (below) need to read it — initializing it at its first append site inside Step 5h, as an LLM-executed runbook literally reads `X = []`, would look like a fresh empty list on every Step 5h entry and silently drop earlier merges from both of those readers.

Also initialize `REGRESSION_PASSES_REMAINING = 3` here — a small, run-wide cap on how many times Step 5i may re-enter after an ISSUES verdict (mirroring Step 5h's existing two-strike "Bounded attempt" idiom for repeated merge-conflict resolutions). This exists because `TOTAL_CI_BUDGET_REMAINING` only ever charges *CI-wait* time (Step 5f item 4) — never the wall-clock cost of the evaluator calls, fix, and commit a regression cycle itself takes — so on a repo with fast or already-terminal CI, a reviewer that keeps finding plausible-but-different ISSUES every pass (never repeating verbatim, so the exact-repeat stale-loop dedup in Step 5i step 4 never fires either) can drive many regression-review/fix/push cycles while barely denting the tracked budget. A small re-entry cap bounds this independently of how much (or little) time CI waits actually charge.
...
If there is nothing to fix (no CI failures, no unanswered feedback), `has_merge_conflict` is false, **and the `errors` list is empty**, run Step 5g's checklist condition 4 (the resolved-upstream ahead-commit check, printed as `PASS`/`FAIL`) before declaring anything — a resumed run can find nothing new to fix while a prior, interrupted run still left local commits unpushed on this exact branch, and that would otherwise slip straight past every other gate below. If it prints `FAIL`, go to Step 5d to push — with Step 5d checking for ahead commits directly, it pushes these even though `COMMITTED_ITEMS` is empty this iteration, then falls through to 5e (a no-op here), 5f (wait for the CI this push triggers), and 5g, which re-runs this same condition as part of its own checklist; this paragraph itself is not re-entered. Once condition 4 prints `PASS`, report the PR is clean and proceed to Step 6 (monitoring). If `has_merge_conflict` is true, go to Step 5h even when there is no other work. If `errors` is non-empty — including an indeterminate `mergeable == null` recorded in Step 3, or any failed Step 3 fetch — do **not** declare the PR clean and do **not** enter monitoring; fall through to Step 5g's retry path (report the fetch failures and retry after 30 seconds) so the ambiguous state resolves before any success exit. A false "clean" here would otherwise skip Step 5g's error gate entirely under `--monitor 0`.
...
1. **Primary Evaluator.** If `SUBAGENT_TOOL`, spawn one clean-context subagent per item with the `Agent` tool. If not `SUBAGENT_TOOL`, evaluate the item yourself, inline, instead — slower, not different.

   Provide the comment, code context, PR title/description, and changed files summary. Ask for (or, inline, produce) a **FIX / DEFER / REJECT** verdict with category, **confidence** (high/low), and reasoning. See `references/comment-evaluation.md` for the full prompt template. Collect every item's verdict before moving on to the advisor check below — do not consult the advisor per item.

2. **Advisor check** (only if `ADVISOR`). Once every feedback item in this pass has a Primary verdict, state them all in-transcript in one batch — item key, the comment being evaluated, the Primary verdict, its confidence, and its reasoning — then consult the advisor once for the whole batch. The advisor takes no separate prompt; it reviews the conversation as it stands, so the batch must already be stated before you call it. Ask it (in the same in-transcript framing) to flag any item where it would land on a different verdict, and why.

   If `ADVISOR` is false, skip this call entirely — every item's Primary verdict is final, subject to the confidence rule below.

**Decision logic** (from `references/comment-evaluation.md`):

| Primary | Advisor (if consulted) | Action |
|---------|-------------------------|--------|
| FIX | FIX (or not flagged) | **FIX** — apply code change in this PR |
| FIX | DEFER | **FIX** — both consider the feedback valid; apply it now instead of filing an issue |
| DEFER | FIX | **FIX** — same as above, either order |
...
| any other disagreement (either side REJECT, the other FIX or DEFER) | | **DEFER** — file tracking issue |
| any verdict, `ADVISOR` false, **low confidence** | — | **DEFER** — file tracking issue |

The rule leans toward action without being reckless: fix when the evaluator(s) agree the change belongs in this PR, and also on a FIX/DEFER split — both consider the feedback valid, so the disagreement is only about timing and fixing now avoids issue churn. Only reject when there is no advisor disagreement and no concern worth tracking. Every remaining disagreement involving a REJECT vote files an issue so nothing is silently dropped. When no advisor is available to catch a shaky call, a **low-confidence** Primary verdict routes to DEFER instead of being acted on unchecked — this is the compensating control for running without a second opinion; a **high-confidence** Primary verdict with no advisor available stands as given. This matches the "Three Outcomes" core principle.
...
_This assessment was made by {if ADVISOR: an AI reviewer, cross-checked by an advisor | else: an AI reviewer}. If you disagree, please reply and we'll re-evaluate._
```
...
2. Build the issue body:
   ...
   **Why deferred:** {scope-creep | diminishing-returns | ambiguous} — {one-sentence rationale from the evaluation}.

   _Filed automatically by `pm-autofix-pr` after triage{if ADVISOR: , cross-checked by an advisor}._
   ```
...
6. **On pre-commit failure** (5c returned a hard fail after the sub-fix attempt): revert this item completely so the next FIX starts from a clean worktree:
   ...
   Record the item under `BLOCKED_ITEMS = []` with `{item_key, files, pre_commit_error_tail}` — plus `finding_summary` when `item_key` starts with `regression:` (carried over from the `THIS_PASS_FINDINGS` entry Step 5i step 4 is routing; every other item_key has no `finding_summary` of its own and omits the field). Continue with the next FIX item; blocked items will be turned into `automated-fix-failed` DEFER entries (with their own tracking issues) at the end of the loop iteration.
...
**Convert each blocked FIX into an `automated-fix-failed` DEFER before leaving 5b — except synthetic regression items.** A `BLOCKED_ITEMS` entry whose `item_key` starts with `regression:` (Step 5i's self-caught findings, routed through this same flow) cannot go through the DEFER conversion below: that conversion quotes "the reviewer's original feedback" and posts a reply on "the original thread / review summary / PR conversation comment," and a synthetic regression item has neither — there is no reviewer text to quote and no thread to reply on. Pull these out of `BLOCKED_ITEMS` first and append them (with their carried-over `finding_summary`, above) to the run-wide `BLOCKED_REGRESSIONS` (initialized in Step 0b) instead; do not run the DEFER flow for them. The run then falls through to 5d/5e for whatever *did* commit this pass (if anything) and exits directly to Step 7 with `exit reason: regression-fix-failed` — see Step 5i step 4's own routing for this exit and for how `LAST_REGRESSION_FINDINGS` is handled on a pass with blocked items; this paragraph only covers the reply-target carve-out, not the exit itself.

For every remaining entry in `BLOCKED_ITEMS` (the reviewer-triggered items 5b reverted because pre-commit refused them), run the Step 5a' DEFER flow with `category="automated-fix-failed"`:
...
- Check for ahead commits directly — using the same upstream-resolution Step 5g's condition 4 uses (resolve `@{u}`; with no upstream configured, treat every local commit as ahead) — rather than relying on `COMMITTED_ITEMS` alone: it is scoped to this iteration and reads empty whenever Step 5b didn't run this iteration, even when a *previous*, interrupted run already made commits that were never pushed (the case Step 3's "nothing to fix" short-circuit routes here to handle). If `COMMITTED_ITEMS` is empty for this iteration **and** there are no ahead commits, there is nothing to push — **skip directly to Step 5g** (re-fetch and check fixed point), **not** 5f. An iteration that only emits REJECT replies, DEFER replies, or `automated-fix-failed` DEFER conversions creates no new commits and therefore no new CI events, so waiting for CI in 5f would just spin until `CI_TIMEOUT` and exit as a failure even when we have actually reached the fixed point.
- Otherwise (either `COMMITTED_ITEMS` is non-empty, or ahead commits exist despite this iteration producing none), push:
...
**5e. Reply to every addressed feedback item.** For each entry in `COMMITTED_ITEMS` (the per-item commits 5b/5d produced this iteration), post an outcome reply that quotes that item's own short-sha — never a different item's sha, since each FIX has its own commit. An item only counts as addressed once its reply is posted. Skip this for entries whose `item_key` starts with `regression:` (Step 5i's self-caught fixes, routed through this same commit flow with a synthetic key) — there is no reviewer thread to reply to for a bug the loop found in its own commits; those are recorded in `REGRESSION_FIXES` and surfaced in Step 7's summary instead.
...
This wait-and-refetch path is the only mechanism the skill uses to detect new state. It does not depend on `<github-webhook-activity>` envelopes or any subscription tool; those are not available in the interactive CLI coding sessions this skill targets. `gh pr checks --watch` changes how the wait is spent, not how state is detected.
...
**Fixed-point checklist.** Every time 5g runs, evaluate all five conditions below and print each as `PASS`/`FAIL` with the concrete evidence that proves it — not just the verdict — with one exception: condition 5 has a third possible print, `DEGRADED (PASS)` (see condition 5's own text), which counts as a `PASS` for every purpose below (the fixed-point gate, the exit-reason selection, and Step 7's summary) but is called out separately so a degraded single-evaluator confirmation stays visible rather than reading as an ordinary two-evaluator `PASS`. This is what keeps a resumed or reviewed run legible instead of trailing off mid-analysis with no record of what was actually still open:
...
4. **Zero unpushed commits** — resolve the branch's actual upstream the same way Step 5d does: run `git rev-parse --abbrev-ref --symbolic-full-name @{u}`. If it exits `0`, compare with `git log @{u}..HEAD --oneline`; a non-zero exit from that `git log` is `FAIL`, never `PASS` — a fatal error and a genuinely clean result both print empty stdout, so the exit status decides it, not the output alone. If the `@{u}` resolution itself exits non-zero, the branch has no upstream yet — treat every local commit as unpushed (`FAIL` whenever `git log HEAD --oneline` is non-empty). Print the resolved ref (or "no upstream") and the command's output verbatim as evidence. Empty output on a zero exit is `PASS`; anything else is `FAIL` — return to 5d to push, then re-run this checklist. This check exists because control flow alone (5d pushes before 5g runs) is not proof: a prior run's push can fail, be skipped, or be interrupted in a way this explicit check catches even when every other signal looks clean — this is what a stale unpushed-commits state from a prior run looks like from inside 5g.
5. **No regressions in this run's own commits** — only evaluate this condition once conditions 1-4 above already print `PASS` (no point spending a review pass on a diff that CI, feedback triage, or a conflict merge is about to change anyway). If **both** `ALL_COMMITTED_ITEMS` and `MERGE_COMMITS` are empty (this invocation hasn't authored any commit of its own yet — e.g. Step 4's "nothing to fix" short-circuit before any iteration ran, and no base-branch conflict was ever merged in), print `PASS` vacuously without spawning a reviewer: there is nothing of *this run's* authorship to check for self-inflicted regressions, and CI already vets whatever commits already existed on the branch coming in. A non-empty `MERGE_COMMITS` alone (a run that only resolved a base-branch conflict, with no Step 5b FIX) is **not** vacuous — Step 5h's hand-resolved conflicts are exactly the kind of self-authored, regression-prone change this condition exists to catch, per this PR's own motivating incidents. Otherwise run Step 5i's regression-review pass. Print `PASS` when every call this pass made agreed CLEAN, or `DEGRADED (PASS)` when Step 5i step 3's no-advisor or no-verdict handling resolved this pass without a finding (see step 3); print `FAIL` with the combined finding list when any call returns ISSUES, then follow Step 5i's routing rather than converging.

**Fixed point reached** only when all five conditions above print `PASS` (condition 5's `DEGRADED (PASS)` counts as `PASS` here — see the checklist's own note above) → proceed to Step 6.

**Merge conflict present** (`has_merge_conflict` true) → run Step 5h, then continue the loop.

**Stale loop detected** if the same CI checks are failing with similar error patterns as the previous iteration, **or** two consecutive iterations resolved base-branch conflicts yet the PR is still reported non-mergeable, **or** Step 5i reports the same regression finding twice in a row despite an intervening fix, **or** Step 5i's `REGRESSION_PASSES_REMAINING` re-entry cap reaches zero on a pass with genuinely new (never-repeating) findings → record `stale_loop` in the final summary and jump to Step 7. Do not prompt the user. (`BLOCKED_ITEMS` cannot accumulate across iterations because Step 5b now converts each blocked FIX into an `automated-fix-failed` DEFER and clears the list.)
...
6. **Record it.** Append `{sha, base_ref, conflicted_files}` to the run-wide `MERGE_COMMITS` (initialized in Step 0b) for the Step 7 summary, then continue the loop (return to Step 5f to wait for the new CI run, or Step 5g to re-check the fixed point).
...
Runs only when Step 5g's conditions 1-4 already print `PASS` and at least one of `ALL_COMMITTED_ITEMS` or `MERGE_COMMITS` is non-empty (see condition 5's own text for the vacuous-empty case, and for why no point reviewing a diff that other in-flight work is about to change).

**Both passes happen in this single invocation, not across separate loop iterations.** An earlier version of this step persisted a "clean streak" across 5g re-entries — the first CLEAN verdict left the streak at 1, condition 5 kept printing `FAIL`, and control had nowhere productive to go: no new commit had landed, so Step 5f's wait-for-external-change primitive had nothing to wait on and the run spun until `ci-timeout`. Getting independent verification without that dead wait means reviewing the diff fully *now*, not the same commits reviewed twice, later:

1. **Run the Primary Evaluator**, reusing Step 4's evaluator infrastructure (Step 0a's `SUBAGENT_TOOL` flag: a fresh, clean-context `Agent`-tool spawn if true, an inline self-review if false) for a review of the diff of `ALL_COMMITTED_ITEMS` **and** `MERGE_COMMITS` combined (every commit this invocation has authored so far, run-wide — not just this iteration's, and not just Step 5b's FIXes). For each `ALL_COMMITTED_ITEMS` sha, diff it normally; for each `MERGE_COMMITS` sha, use `git show --remerge-diff <sha>` (git ≥ 2.36; the `ort` merge strategy that Step 5h's plain `git merge`/`git commit` already use) rather than a plain diff, `git log -p`, or `git show --cc`. A bare diff against a merge commit's first parent silently omits the hand-resolved hunks entirely, and `--cc` (dense combined diff) has its own silent gap: it suppresses any region where the merge result matches either parent, so a resolution that took one side **wholesale** — exactly the failure mode Step 5h's own instructions warn against ("Never blindly keep one side") and this condition exists to catch — produces zero diff hunks, letting the review return CLEAN on an empty input rather than on an actual review. `--remerge-diff` instead shows the real diff between the machine-remerged tree and what was actually committed, so a wholesale-side resolution (or any other silent drop) is visible. If the resulting diff for a given sha is genuinely empty (a clean, unmodified auto-merge Step 5h never hand-touched), evaluate it as CLEAN directly rather than passing an empty diff into evaluation — an empty prompt input risks tripping the no-verdict path in step 3 below into a spurious `DEGRADED (PASS)`. On git < 2.36 (`--remerge-diff` unsupported), fall back to `--cc` and accept its wholesale-side blind spot as a known limitation rather than blocking the review. Reviewing only `ALL_COMMITTED_ITEMS` would mean a run that only resolved a base-branch conflict never gets its hand-authored resolution reviewed at all (see Step 5g condition 5's non-vacuous case above); reviewing `start-of-run..HEAD` instead of these two specific sets was considered and rejected, because after a base merge that range also carries the entire base branch's own delta, which would have the evaluator flag other contributors' pre-existing code as this run's regressions. A clean-context spawn reviewing a diff it did not author is a stronger independence property than the orchestrator grading its own commits from the same contaminated context that produced them, and it costs one bounded subagent call per 5g pass (plus, if `ADVISOR`, one advisor consultation) instead of an open-ended wait.
2. **Prompt**: review only for correctness bugs and regressions the fix commits themselves introduced — not style, not scope, not anything Step 4's evaluation already triages from reviewer feedback. Explicitly ask the Primary Evaluator to check for, at minimum:
...
3. **Combine.** If `ADVISOR`: state the Primary Evaluator's CLEAN/ISSUES verdict and its findings in-transcript, then consult the advisor once, asking it to independently confirm CLEAN or flag ISSUES against the same diff. When both agree CLEAN, condition 5 is satisfied for this 5g pass — nothing further to do. When either is ISSUES, union their findings into `THIS_PASS_FINDINGS`, deduping entries that name the same file:line and the same defect. If `ADVISOR` is false, the Primary Evaluator's verdict is used alone: CLEAN satisfies condition 5 but is marked degraded (single-evaluator confirmation only — see `DEGRADED_EVALUATORS` below); ISSUES populates `THIS_PASS_FINDINGS` directly, not marked degraded — a finding needing a fix stands on its own regardless of how many reviewers produced it.

   **No-verdict evaluator.** Any evaluation call this step makes (the Primary Evaluator, or the advisor consultation) that times out, exits nonzero, or returns output with no parseable CLEAN/ISSUES verdict has produced no usable input. Retry that one call once (same framing, same diff). If the retry also produces no parseable verdict:
   - If the other call (Primary or advisor, whichever is still live) has a valid verdict, proceed on it alone: CLEAN → condition 5 passes, marked degraded; ISSUES → its findings alone populate `THIS_PASS_FINDINGS` and proceed to step 4 below as usual.
   - If no call this pass produced a parseable verdict at all — the Primary Evaluator failed with no advisor available to fall back on, or both the Primary Evaluator and the advisor failed — condition 5 is a degraded PASS with zero findings for this pass. Do not spin retrying indefinitely, and do not treat a non-verdict as ISSUES.

   Set `DEGRADED_EVALUATORS` (per-pass state, overwritten each time this step runs — not a run-wide accumulator like `LAST_REGRESSION_FINDINGS`) to record why this pass is degraded: `no-advisor` when `ADVISOR` is false and the Primary Evaluator alone confirmed CLEAN, or the identity of whichever call (`primary` / `advisor`) produced no verdict after its retry — so Step 7's "Regression review" status line can report `DEGRADED (<DEGRADED_EVALUATORS>)` instead of a plain `PASS`; leave it empty on any pass where every call this step made returned a parseable verdict.
4. **On any ISSUES:** compare `THIS_PASS_FINDINGS` against `LAST_REGRESSION_FINDINGS` (run-wide state from Step 0b, holding whatever the previous Step 5i invocation last found — empty if this is the first). If they match — the fix applied last time didn't actually resolve what was flagged — that is the stale-loop trigger cross-referenced from Step 5g's "Stale loop detected" bullet: record `stale_loop` and jump to Step 7 rather than looping forever chasing a fix the reviewer keeps rejecting. Otherwise, decrement `REGRESSION_PASSES_REMAINING` (initialized in Step 0b). If it has reached zero, this pass's ISSUES are *different* findings than last time (the exact-repeat dedup above didn't catch them) but the run has still re-entered this step too many times — record `stale_loop` with `THIS_PASS_FINDINGS` listed as the outstanding findings and jump to Step 7, rather than treating "never repeats verbatim" as license to keep going indefinitely. Otherwise, treat each finding in `THIS_PASS_FINDINGS` as its own FIX item with a synthetic `item_key` of `regression:<n>` (no reviewer thread backs it) — no Step 4 triage, no FIX/DEFER/REJECT ambiguity call: the loop authored the bug, so the loop fixes it, unconditionally. Route each through Step 5b's existing single-item flow (apply, 5c pre-commit, commit — which appends the usual `{item_key, sha, files, validation}` entry to `COMMITTED_ITEMS`/`ALL_COMMITTED_ITEMS`, same as any other FIX), additionally append `{sha, finding_summary}` to `REGRESSION_FIXES` for Step 7's dedicated table. Once every FIX item from this pass has been attempted, set `LAST_REGRESSION_FINDINGS` to whichever findings in `THIS_PASS_FINDINGS` landed a commit (all of them, on the common all-succeeded path) — this is what lets the *next* Step 5i invocation detect an exact repeat; skipping this update on the success path would silently disable the exact-repeat stale-loop trigger for every future pass. Then go to 5d to push (5d sees `COMMITTED_ITEMS` non-empty from these entries and pushes normally) and 5e (which skips the reply for any `regression:` key — see 5e). The next 5g pass re-runs this regression review from scratch against the updated diff.

   **If any regression item is blocked** (5c's pre-commit check still fails after its one sub-fix), Step 5b routes it to `BLOCKED_REGRESSIONS` instead of the normal `automated-fix-failed` DEFER conversion — see Step 5b's carve-out for why a synthetic finding has no reviewer thread to reply on. `LAST_REGRESSION_FINDINGS` is still only set to whichever findings landed a commit (the general rule above already excludes a blocked finding, since it never lands); if *every* finding this pass blocked, `LAST_REGRESSION_FINDINGS` is unchanged from its prior value — a pass that landed nothing didn't actually resolve (or fail to resolve) anything, so overwriting it would corrupt a later resumed run's stale-loop comparison. Once every FIX item from this pass has been attempted, a non-empty `BLOCKED_REGRESSIONS` means the run stops here — do not re-enter Step 5i again this invocation. Fall through to 5d/5e for whatever did commit (if anything, per the general routing above), then exit directly to Step 7 with `exit reason: regression-fix-failed` (do not route back through Step 5g's condition 5 — see condition 5's own text, which delegates unconditionally to "Step 5i's routing" and defers to this exit rather than defining its own): the loop authored these bugs and could not land its own fix, so it surfaces the failure rather than guessing further.

**Bound.** Beyond the exact-repeat stale-loop trigger in this step's own numbered step 4 above, a reviewer that keeps finding *different* plausible-sounding issues every pass (never repeating verbatim) isn't caught by that dedup. `TOTAL_CI_BUDGET_REMAINING` is **not** a reliable backstop for this case: it only ever charges the *wait* portion of Step 5f (item 4), never the wall-clock cost of Step 5i's own evaluator calls, Step 5b's fix/commit, or Step 5d's push — so on a repo with fast or already-terminal CI, a cycle can charge only seconds to the budget while actually costing much more real time, letting many more regression/fix/push cycles happen than the configured `TOTAL_CI_BUDGET` minutes would suggest. `REGRESSION_PASSES_REMAINING` (initialized to 3 in Step 0b, decremented in step 4 above) is what actually bounds this case: once it reaches zero, the run exits via `stale-loop` regardless of how much CI-wait budget remains, the same way Step 5h's "Bounded attempt" caps repeated merge-conflict resolutions.

### Step 6: Monitoring Phase

Skip if `MONITOR_DURATION` is 0 or if there are CI failures, unanswered feedback items, or a merge conflict (`has_merge_conflict` true). Monitoring is only entered from a true fixed point — which, per Step 5g, already means condition 5's regression review resolved (vacuously or otherwise) within that same 5g pass, so no separate regression check is needed here.
...
### PR: #<number> — <title>
### Iterations: N
### Exit reason: fixed-point | stale-loop | ci-timeout | merge-conflict | rebase-conflict | push-failure | dirty-worktree | monitoring-timeout | regression-fix-failed
...
### Blocked Regression Fixes (self-caught by Step 5i, pre-commit failed — `exit reason: regression-fix-failed`)
| Finding | Files | Pre-commit failure |
|---------|-------|---------------------|
| Hardcoded expected hash drifted once `main` advanced | test/regression_test.py | test: assertion mismatch |
...
- Unpushed commits: PASS (no commits ahead of the resolved upstream) / FAIL (list the commit shas + subjects still ahead — or, with no upstream configured, every local commit)
- Regression review: PASS (Primary, and the advisor if consulted, agreed CLEAN) / DEGRADED (`DEGRADED_EVALUATORS` reason — no advisor available, or a call returned no parseable verdict after retry) / vacuous (no commits this run) / FAIL (list the latest combined findings)
- Issue creation failures: 0 / K (each requires manual filing — see Deferred table)
```

The last Step 5g checklist evaluation (or, for any exit that never reached 5g — e.g. `dirty-worktree`/`push-failure`, an early `ci-timeout` on a first push, a `merge-conflict` failing out of 5h, or a `rebase-conflict` failing out of 5d — a fresh run of its five checks) is the source for this block — every exit prints the same five PASS/FAIL conditions, not just `fixed-point`, so a `stale-loop` or `ci-timeout` exit is exactly as legible as a success.
...
- **Failure exits** — `stale-loop`, `ci-timeout` (also posts the budget-exhaustion PR comment above), `merge-conflict`, `rebase-conflict`, `push-failure`, `dirty-worktree`, `regression-fix-failed`: print **"Autofix exited without converging — see summary above for required follow-up."** For a `merge-conflict` exit, add: **"Merge conflicts with `<base.ref>` could not be resolved automatically — resolve them manually, then re-run."** For a `regression-fix-failed` exit, add: **"A self-caught regression fix in `<sha or 'the working tree' if reverted>` failed pre-commit and could not be landed automatically — see the Blocked Regression Fixes table above, fix it manually, then re-run."** Do not loop again, do not prompt.
