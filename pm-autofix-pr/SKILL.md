...
**Both passes happen in this single invocation, not across separate loop iterations.** An earlier version of this step persisted a "clean streak" across 5g re-entries — the first CLEAN verdict left the streak at 1, condition 5 kept printing `FAIL`, and control had nowhere productive to go: no new commit had landed, so Step 5f's wait-for-external-change primitive had nothing to wait on and the run spun until `ci-timeout`. Getting independent verification without that dead wait means running two independent reviewers *now*, not the same reviewer twice, later:

1. **Spawn both evaluators in parallel**, reusing Step 4's dual-evaluator infrastructure — Step 0a's **Local Evaluator** and **Cross-harness Evaluator** rows (Agent tool `model="opus"` + Skill tool `skill="codex-2nd-opinion"` on Claude host; `codex exec --sandbox read-only --ephemeral` + `claude -p --permission-mode auto` on Codex host, same `mktemp`/`rm -f` discipline as Step 4) — each a fresh, independent, clean-context review of the diff of `ALL_COMMITTED_ITEMS` **and** `MERGE_COMMITS` combined (every commit this invocation has authored so far, run-wide — not just this iteration's, and not just Step 5b's FIXes). For each `ALL_COMMITTED_ITEMS` sha, diff it normally; for each `MERGE_COMMITS` sha, use `git show --remerge-diff <sha>` (git ≥ 2.36; the `ort` merge strategy that Step 5h's plain `git merge`/`git commit` already use) rather than a plain diff, `git log -p`, or `git show --cc`. A bare diff against a merge commit's first parent silently omits the hand-resolved hunks entirely, and `--cc` (dense combined diff) has its own silent gap: it suppresses any region where the merge result matches either parent, so a resolution that took one side **wholesale** — exactly the failure mode Step 5h's own instructions warn against ("Never blindly keep one side") and this condition exists to catch — produces zero diff hunks, letting both evaluators return CLEAN on an empty input rather than on an actual review. `--remerge-diff` instead shows the real diff between the machine-remerged tree and what was actually committed, so a wholesale-side resolution (or any other silent drop) is visible. If the resulting diff for a given sha is genuinely empty (a clean, unmodified auto-merge Step 5h never hand-touched), evaluate it as CLEAN directly rather than passing an empty diff to the evaluators — an empty prompt input risks tripping the no-verdict path in step 3 below into a spurious `DEGRADED (PASS)`. On git < 2.36 (`--remerge-diff` unsupported), fall back to `--cc` and accept its wholesale-side blind spot as a known limitation rather than blocking the review. Reviewing only `ALL_COMMITTED_ITEMS` would mean a run that only resolved a base-branch conflict never gets its hand-authored resolution reviewed at all (see Step 5g condition 5's non-vacuous case above); reviewing `start-of-run..HEAD` instead of these two specific sets was considered and rejected, because after a base merge that range also carries the entire base branch's own delta, which would have the evaluators flag other contributors' pre-existing code as this run's regressions. Two different model families reviewing independently is a stronger independence property than the same model spawned twice, and it costs one bounded pair of subagent calls per 5g pass instead of an open-ended wait.
2. **Prompt** (identical for both evaluators): review only for correctness bugs and regressions the fix commits themselves introduced — not style, not scope, not anything Step 4's dual-evaluator already triages from reviewer feedback. Explicitly ask each to check for, at minimum:
...
3. **Combine.** If both evaluators return a parseable CLEAN/ISSUES verdict: when both are CLEAN, condition 5 is satisfied for this 5g pass — nothing further to do. When either is ISSUES, union their findings into `THIS_PASS_FINDINGS`, deduping entries that name the same file:line and the same defect.

   **No-verdict evaluator.** An evaluator that times out, exits nonzero, or returns output with no parseable CLEAN/ISSUES verdict has produced neither branch's input above — a plain "both CLEAN or either ISSUES" reading leaves this case undefined and would strand an otherwise-converged run. Retry that evaluator once (same prompt, same diff). If the retry also produces no parseable verdict, proceed on the surviving evaluator's verdict alone: CLEAN → condition 5 passes but is marked degraded (single-evaluator confirmation only, not the normal two-evaluator independence guarantee); ISSUES → its findings alone populate `THIS_PASS_FINDINGS` and proceed to step 4 below as usual. If *both* evaluators fail to produce a parseable verdict even after their individual retries, condition 5 is a degraded PASS with zero findings for this pass — do not spin retrying indefinitely, and do not treat a doubled non-verdict as ISSUES. Set `DEGRADED_EVALUATORS` (per-pass state, overwritten each time this step runs — not a run-wide accumulator like `LAST_REGRESSION_FINDINGS`) to the list of evaluator label(s) that produced no verdict this pass, so Step 7's "Regression review" status line can report `DEGRADED (<DEGRADED_EVALUATORS> returned no verdict)` instead of a plain `PASS`; leave it empty on any pass where both evaluators returned a parseable verdict.
4. **On any ISSUES:** compare `THIS_PASS_FINDINGS` against `LAST_REGRESSION_FINDINGS` (run-wide state from Step 0b, holding whatever the previous Step 5i invocation last found — empty if this is the first). If they match — the fix applied last time didn't actually resolve what was flagged — that is the stale-loop trigger cross-referenced from Step 5g's "Stale loop detected" bullet: record `stale_loop` and jump to Step 7 rather than looping forever chasing a fix the reviewer keeps rejecting. Otherwise, decrement `REGRESSION_PASSES_REMAINING` (initialized in Step 0b). If it has reached zero, this pass's ISSUES are *different* findings than last time (the exact-repeat dedup above didn't catch them) but the run has still re-entered this step too many times — record `stale_loop` with `THIS_PASS_FINDINGS` listed as the outstanding findings and jump to Step 7, rather than treating "never repeats verbatim" as license to keep going indefinitely. Otherwise, treat each finding in `THIS_PASS_FINDINGS` as its own FIX item with a synthetic `item_key` of `regression:<n>` (no reviewer thread backs it) — no Step 4 triage, no FIX/DEFER/REJECT ambiguity call: the loop authored the bug, so the loop fixes it, unconditionally. Route each through Step 5b's existing single-item flow (apply, 5c pre-commit, commit — which appends the usual `{item_key, sha, files, validation}` entry to `COMMITTED_ITEMS`/`ALL_COMMITTED_ITEMS`, same as any other FIX), additionally append `{sha, finding_summary}` to `REGRESSION_FIXES` for Step 7's dedicated table. Once every FIX item from this pass has been attempted, set `LAST_REGRESSION_FINDINGS` to whichever findings in `THIS_PASS_FINDINGS` landed a commit (all of them, on the common all-succeeded path) — this is what lets the *next* Step 5i invocation detect an exact repeat; skipping this update on the success path would silently disable the exact-repeat stale-loop trigger for every future pass. Then go to 5d to push (5d sees `COMMITTED_ITEMS` non-empty from these entries and pushes normally) and 5e (which skips the reply for any `regression:` key — see 5e). The next 5g pass re-runs this dual-evaluator review from scratch against the updated diff.
...
Skip if `MONITOR_DURATION` is 0 or if there are CI failures, unanswered feedback items, or a merge conflict (`has_merge_conflict` true). Monitoring is only entered from a true fixed point — which, per Step 5g, already means condition 5's dual-evaluator regression review resolved (vacuously or otherwise) within that same 5g pass, so no separate regression check is needed here.
...
- Regression review: PASS (both evaluators returned CLEAN) / DEGRADED (one or both evaluators returned no parseable verdict after retry — list which) / vacuous (no commits this run) / FAIL (list the latest combined findings)
