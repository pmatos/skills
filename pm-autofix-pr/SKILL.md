...
3. **Combine.** If both evaluators return CLEAN, condition 5 is satisfied for this 5g pass — nothing further to do. If either returns ISSUES, union their findings into `THIS_PASS_FINDINGS`, deduping entries that name the same file:line and the same defect.
...
- Regression review: PASS (both evaluators returned CLEAN) / vacuous (no commits this run) / FAIL (list the latest combined findings)
