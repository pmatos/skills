...
**Convert each blocked FIX into an `automated-fix-failed` DEFER before leaving 5b — except synthetic regression items.** A `BLOCKED_ITEMS` entry whose `item_key` starts with `regression:` (Step 5i's self-caught findings, routed through this same flow) cannot go through the DEFER conversion below: that conversion quotes "the reviewer's original feedback" and posts a reply on "the original thread / review summary / PR conversation comment," and a synthetic regression item has neither — there is no reviewer text to quote and no thread to reply on. Pull these out of `BLOCKED_ITEMS` first and append `{item_key, files, pre_commit_error_tail, finding_summary}` to the run-wide `BLOCKED_REGRESSIONS` (initialized in Step 0b) instead; do not run the DEFER flow for them and do not set `LAST_REGRESSION_FINDINGS` from a pass whose fixes all blocked (see Step 5i step 4 — leaving it set from a pass that landed nothing would corrupt a later resumed run's stale-loop comparison). The run then falls through to 5d/5e for whatever *did* commit this pass (if anything), and Step 5g's condition 5 exits through Step 7 with `exit reason: regression-fix-failed` rather than looping again — the loop authored these bugs and could not land its own fix, so it stops and surfaces the failure instead of guessing further.
