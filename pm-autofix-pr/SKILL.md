...
**Bound.** Beyond the exact-repeat stale-loop trigger in this step's own numbered step 4 above, a reviewer that keeps finding *different* plausible-sounding issues every pass (never repeating verbatim) isn't caught by that dedup — but every ISSUES verdict forces a real fix-and-push, and the resulting Step 5f CI wait still deducts from `TOTAL_CI_BUDGET_REMAINING` like every other wait in this loop, so that case still bottoms out in the existing `ci-timeout` exit rather than running forever.
