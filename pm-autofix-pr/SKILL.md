...
**Stale loop detected** if the same CI checks are failing with similar error patterns as the previous iteration, **or** two consecutive iterations resolved base-branch conflicts yet the PR is still reported non-mergeable, **or** Step 5i reports the same regression finding twice in a row despite an intervening fix → record `stale_loop` in the final summary and jump to Step 7. Do not prompt the user. (`BLOCKED_ITEMS` cannot accumulate across iterations because Step 5b now converts each blocked FIX into an `automated-fix-failed` DEFER and clears the list.)
