...
- **Adopts the branch it was handed**, when that branch is non-default and has no commits of its own — the state a headless harness leaves a prepared workspace in. Such harnesses look for the resulting PR by its head branch, so keeping their name is what makes the PR visible to whatever scheduled the run; otherwise the skill cuts its own branch from `origin/<default-branch>`. It never works on the default branch, and never prunes a branch it did not create.
