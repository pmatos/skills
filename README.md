...
What it does:
- Gathers the diff under review (`git diff <base>...HEAD` against the resolved default branch — `origin/HEAD` symref, then `gh repo view --json defaultBranchRef`, then a local `main`/`master`, then `@{upstream}`, with `HEAD~1` only as a disclosed last resort — plus any uncommitted working-tree changes), or reviews an explicit PR/branch/path argument.
