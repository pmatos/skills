...
- **Adopts the branch it was handed**, when that branch is provably its own to take: non-default, no unique history, no upstream, and unpublished on origin — the state a headless harness leaves a prepared workspace in. Such harnesses look for the resulting PR by its head branch, so keeping their name is what makes the PR visible to whatever scheduled the run. Emptiness alone is not enough (a merged topic branch is empty too, and shared), so every condition fails closed to cutting its own branch from `origin/<default-branch>`. It never works on the default branch, and never prunes a branch it did not create.
